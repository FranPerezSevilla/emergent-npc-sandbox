...
const followUpCue = /\b(?:rumor|rumour|rumorea|dicen|dijo|dijeron|cont[oó]|contaron|qu[ié]n|who|what did|eso|that)\b/i;
...
const relevantConflictFactIds = (
  ...
  return [...beliefsByFact.entries()]
    ...
    .filter(([factId]) => {
      ...
      return mentionsAny(playerUtterance, terms) || (isFollowUp && mentionsAny(recentText, terms));
    })
    .map(([factId]) => factId);
};
