# Third-Party Notices

<!-- GENERATED FROM legal/third-party.json. Do not manually add authoritative records here. -->

This inventory summarizes recorded third-party resources. Full license/notice evidence, when stored in-repository, lives under `legal/licenses/`.

## @mlc-ai/web-llm

- ID: `webllm-0.2.82`
- Type: `software`
- Creator/provider: MLC community and contributors
- License/terms: Apache License 2.0
- License/terms URL: https://github.com/mlc-ai/web-llm/blob/v0.2.82/LICENSE
- Source URL: https://github.com/mlc-ai/web-llm/tree/v0.2.82
- Commercial use: allowed
- Modification: allowed
- Redistribution: allowed
- AI processing: not_applicable
- Release allowed: yes
- Evidence: legal/licenses/webllm-0.2.82-Apache-2.0.txt, https://github.com/mlc-ai/web-llm/blob/v0.2.82/LICENSE, https://www.npmjs.com/package/@mlc-ai/web-llm/v/0.2.82

## actions/checkout

- ID: `github-actions-checkout-v7`
- Type: `software`
- Creator/provider: GitHub, Inc. and contributors
- License/terms: MIT License
- License/terms URL: https://github.com/actions/checkout/blob/v7/LICENSE
- Source URL: https://github.com/actions/checkout
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: legal/licenses/github-actions-checkout-v7-MIT.txt, https://github.com/actions/checkout/blob/v7/LICENSE

## actions/configure-pages

- ID: `github-actions-configure-pages-v6`
- Type: `software`
- Creator/provider: GitHub, Inc.
- License/terms: MIT License
- License/terms URL: https://github.com/actions/configure-pages/blob/v6/LICENSE
- Source URL: https://github.com/actions/configure-pages
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/actions/configure-pages/blob/v6/LICENSE

## actions/deploy-pages

- ID: `github-actions-deploy-pages-v5`
- Type: `software`
- Creator/provider: GitHub, Inc.
- License/terms: MIT License
- License/terms URL: https://github.com/actions/deploy-pages/blob/v5/LICENSE
- Source URL: https://github.com/actions/deploy-pages
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/actions/deploy-pages/blob/v5/LICENSE

## actions/setup-node

- ID: `github-actions-setup-node-v7`
- Type: `software`
- Creator/provider: GitHub, Inc. and contributors
- License/terms: MIT License
- License/terms URL: https://github.com/actions/setup-node/blob/v7/LICENSE
- Source URL: https://github.com/actions/setup-node
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/actions/setup-node/blob/v7/LICENSE

## actions/upload-pages-artifact

- ID: `github-actions-upload-pages-artifact-v5`
- Type: `software`
- Creator/provider: GitHub, Inc.
- License/terms: MIT License
- License/terms URL: https://github.com/actions/upload-pages-artifact/blob/v5/LICENSE
- Source URL: https://github.com/actions/upload-pages-artifact
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/actions/upload-pages-artifact/blob/v5/LICENSE

## PlayCanvas Engine

- ID: `playcanvas-engine-2.21.4`
- Type: `software`
- Creator/provider: PlayCanvas Ltd.
- License/terms: MIT License
- License/terms URL: https://github.com/playcanvas/engine/blob/v2.21.4/LICENSE
- Source URL: https://github.com/playcanvas/engine/tree/v2.21.4
- Commercial use: allowed
- Modification: allowed
- Redistribution: allowed
- AI processing: not_applicable
- Release allowed: yes
- Evidence: legal/licenses/playcanvas-engine-2.21.4-MIT.txt, https://github.com/playcanvas/engine/blob/v2.21.4/LICENSE

## @playcanvas/eslint-config

- ID: `playcanvas-eslint-config-3.0.0-beta.8`
- Type: `tool`
- Creator/provider: PlayCanvas Ltd.
- License/terms: MIT License
- License/terms URL: https://github.com/playcanvas/eslint-config/blob/v3.0.0-beta.8/LICENSE
- Source URL: https://github.com/playcanvas/eslint-config/tree/v3.0.0-beta.8
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/playcanvas/eslint-config/blob/v3.0.0-beta.8/LICENSE

## ESLint

- ID: `eslint-9.39.5`
- Type: `tool`
- Creator/provider: OpenJS Foundation and other contributors
- License/terms: MIT License
- License/terms URL: https://github.com/eslint/eslint/blob/v9.39.5/LICENSE
- Source URL: https://github.com/eslint/eslint/tree/v9.39.5
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/eslint/eslint/blob/v9.39.5/LICENSE

## TypeScript

- ID: `typescript-6.0.3`
- Type: `tool`
- Creator/provider: Microsoft Corporation and contributors
- License/terms: Apache License 2.0
- License/terms URL: https://github.com/microsoft/TypeScript/blob/v6.0.3/LICENSE.txt
- Source URL: https://github.com/microsoft/TypeScript/tree/v6.0.3
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/microsoft/TypeScript/blob/v6.0.3/LICENSE.txt

## Vite

- ID: `vite-8.2.1`
- Type: `tool`
- Creator/provider: VoidZero Inc. and Vite contributors
- License/terms: MIT License
- License/terms URL: https://github.com/vitejs/vite/blob/v8.2.1/LICENSE
- Source URL: https://github.com/vitejs/vite/tree/v8.2.1
- Commercial use: allowed
- Modification: not_applicable
- Redistribution: not_applicable
- AI processing: not_applicable
- Release allowed: yes
- Evidence: https://github.com/vitejs/vite/blob/v8.2.1/LICENSE

## Qwen2.5-0.5B-Instruct q4f32_1 (MLC)

- ID: `qwen2-5-0-5b-instruct-q4f32-1-mlc`
- Type: `ai_model`
- Creator/provider: Qwen Team / MLC community
- License/terms: Quantized artifact license pending explicit verification; base Qwen2.5-0.5B-Instruct is Apache License 2.0
- License/terms URL: https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct/blob/main/LICENSE
- Source URL: https://huggingface.co/mlc-ai/Qwen2.5-0.5B-Instruct-q4f32_1-MLC
- Commercial use: unclear
- Modification: unclear
- Redistribution: unclear
- AI processing: not_applicable
- Release allowed: no
- Evidence: legal/licenses/qwen2.5-0.5b-instruct-base-Apache-2.0.txt, https://huggingface.co/mlc-ai/Qwen2.5-0.5B-Instruct-q4f32_1-MLC, https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct, https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct/blob/main/LICENSE
